...
export class SupplierService {
  ...
  async create(dto: CreateSupplierDto): Promise<Supplier> {
    try {
      // Hook F4a : si IBAN initial fourni, ouvre la ligne courante
      // dans supplier_iban_history (transaction pour atomicité).
      return await this.prisma.$transaction(async (tx) => {
        const created = await tx.supplier.create({ data: { ...dto } });
        if (created.iban) {
          await tx.supplierIbanHistory.create({
            data: {
              ...
              // changedBy laissé null — l'audit log standard trace l'actor
            },
          });
        }
        return created;
  ...
  async replace(id: string, dto: CreateSupplierDto): Promise<Supplier> {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        await this.historizeIbanIfChanged(tx, before, updated);
        return updated;
      });
    } catch (e) {
      this.handlePrismaWriteError(e, dto.code);
  ...
  /**
   * Hook F4a anti-fraude : si l'IBAN (ou BIC, ou bankName) a changé,
   * clôture la ligne courante de supplier_iban_history (effective_to=now)
   * et insère la nouvelle. Aucune action si rien n'a changé.
   *
   * Appelé dans une transaction Prisma pour garantir l'atomicité.
   */
  private async historizeIbanIfChanged(
    tx: Prisma.TransactionClient,
    before: Pick<Supplier, 'id' | 'iban' | 'bic' | 'bankName'>,
    after: Pick<Supplier, 'id' | 'iban' | 'bic' | 'bankName'>,
  ): Promise<void> {
    ...
    // Clôture la ligne courante (si elle existe)
    await tx.supplierIbanHistory.updateMany({
      where: { supplierId: after.id, effectiveTo: null },
      data: { effectiveTo: new Date() },
    });

    // Nouvelle ligne courante (uniquement si IBAN non null — sinon on
    // ne crée pas de ligne fantôme dans l'historique)
    if (after.iban) {
      await tx.supplierIbanHistory.create({
        data: {
          supplierId: after.id,
