...
-- =========================================================================
-- Sprint S4 / US-030 — Grant Office schema (Note Technique + Eligibility)
-- =========================================================================
-- Bounded context métier IPD (Phase 5A). Tables ENTIÈREMENT nouvelles →
-- aucun impact sur les triggers/CHECK existants. Pattern CREATE ... IF NOT
-- EXISTS partout (additif idempotent). FK vers ref.grant_agreement (nom réel
-- de la table « grant » côté DDL/Prisma GrantAgreement) et auth.app_user.
-- Réf : ADR-006 (Note Technique), ADR-007 (Eligibility Engine).
...
-- 1) expense_nature — catalogue typologique global des natures de dépenses
...
-- 2) overhead_rule — règles d'overhead différenciées par catégorie
...
-- 3) note_technique — entité racine (workflow draft → … → superseded)
...
-- NB : l'index UNIQUE PARTIEL (grant_id) WHERE status='active' (au plus une
-- Note Technique active par convention) est l'objet de US-031, pas posé ici.

-- 4) note_technique_budget_line — association M:N Note Technique ↔ budget_line
...
-- 5) eligibility_rule — règle d'éligibilité par (grant, nature) + plafonds
