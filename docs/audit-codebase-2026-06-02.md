...
| F29 | PPT IPD slide 7 : enforcement éligibilité — 4/7 invariants via `submit()`, 3/7 en attente matérialisation DTO PR | 🟢 SUIVI | Grant Office |
...
## 2bis. Suivi Phase 5A — enforcement éligibilité (ajout Sprint S5)

### 🟢 F29 — PPT IPD slide 7 : 4/7 invariants enforced via `submit()`, moteur prêt pour 7/7 (SUIVI)

Le Sprint S5 (`EligibilityEngine` MVP, ADR-007) ferme la lacune historique
liée à **F1** (refacturation Pasteur Paris) et au catalogue PPT slide 7
« à ne pas faire » — invariants jusqu'ici impossibles à enforce dans le
couple Excel + Sage. État au terme de S5 (cf. `eligibility-end-to-end.integration.spec.ts`, US-050) :

- **Enforced *end-to-end* via `PurchaseRequestService.submit()` (4/7)** :
  PPT-1 nature inéligible (`ELIG_NATURE_NOT_ALLOWED`), PPT-2 hors période
  convention (`ELIG_DATE_OUT_OF_WINDOW`), PPT-3 plafond dépassé
  (`ELIG_LINE_BUDGET_EXCEEDED`), PPT-7 période fiscale close
  (`ELIG_PERIOD_CLOSED`).
- **Règles prêtes mais dormantes via `submit()` (3/7)** — le moteur les
  exécute correctement (prouvé au niveau `EligibilityEngineService.validate()`)
  mais le champ source n'est pas encore transporté/matérialisé :
  - PPT-4 ligne ↔ nature (`ELIG_LINE_NATURE_INCOHERENT`) — `EligibilityContextBuilder`
    pose `budgetLine.category = expenseNature.category` (proxy US-049) tant que
    `ref.budget_line.category` n'est pas matérialisé.
  - PPT-5 refacturation Pasteur Paris (`ELIG_PASTEUR_PARIS_REIMBURSED`, lié à F1) —
    `pr.pasteurParisReimbursed` non transporté par `runEligibilityGate` (US-045).
  - PPT-6 doublon facture inter-projet (`ELIG_CROSS_PROJECT_DUPLICATE`, warning) —
    `pr.supplierInvoiceNumber` non transporté (US-046).
- **Résolution prévue Sprint S6** : matérialiser `pasteurParisReimbursed`,
  `supplierInvoiceNumber` et `budget_line.category` (DDL + DTO PR) → bascule
  automatique en blocage/warning *end-to-end*, le moteur étant déjà en place.
